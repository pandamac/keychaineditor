...
void printUsage() {
    /*该起始序列称为控制序列引导符(CSI，Control Sequence Intro)，通常由'\033['或'\e['代替。
    红色字体(31)，白色背景(47)。选项'-e'为echo命令激活特殊字符的解析器,
     其中，'\033['或'\e['引导转义序列，'m'表示设置属性并结束转义序列。Param为属性值，{...}表示可选(多个参数之间用分号隔开，与顺序无关)
     echo -e "\e[31;47m hello world. \e[0m"   必须以 \e[0m 收尾 要不然会显示出错
     */
...
OSStatus osstatusToHumanReadableString(OSStatus status) {
    switch (status) {
        ...
        case errSecItemNotFound:
            fprintf(stderr, "\e[0;31mThe specified item could not be found in the keychain.\e[0m\n");
            break;
        case errSecAuthFailed:
            fprintf(stderr, "\e[0;31mDid you turn off the passcode on device? The item is no longer available.\e[0m\n");
            fprintf(stderr, "\e[0;31mIf that is not the case, UserPresence is required. Check your device for the prompt.\e[0m\n");
            break;
        case -34018:
            fprintf(stderr, "\e[0;31mError: Client has neither application-identifier nor keychain-access-groups entitlements. Please refer README for further instructions.\e[0m\n");
            break;
            
        default:
            fprintf(stderr, "\e[0;31mUnhandled Error: Please contact developer to report this error. Error code: %d\e[0m\n", (int)status);
            break;
    }
 ...
 */
NSArray* checkForNoDataValue(NSData* dataValue) {
    
    if (dataValue != NULL) {
        NSString *string = [NSString stringWithFormat:@"string: %s",[dataValue bytes]];
        NSString *base64 =  [NSString stringWithFormat:@"base64: %@",[dataValue base64EncodedStringWithOptions:0]];
        NSArray *array = [NSArray arrayWithObjects:string,base64, nil];
        
        return array;
    }
    NSString *str = [[@"null" dataUsingEncoding:NSUTF8StringEncoding] base64EncodedStringWithOptions:0];
...
void prepareJsonOutput(NSDictionary *results, NSString *find) {
    ...
    BOOL flag_isiOS8 = isiOS8();
//    NSLog(@"isiOS8 = %d",flag_isiOS8);
    for (eachItemFromResults in results) {
//        NSLog(@"eachItemFromResults = %@",eachItemFromResults);
//        NSLog(@"[eachItemFromResults objectForKey:(__bridge id)kSecAttrService] = %@",\
//              [eachItemFromResults objectForKey:(__bridge id)kSecAttrService]);
        ...
        [innerJSON setObject:mapKeychainConstants([eachItemFromResults objectForKey:(__bridge id)kSecAttrAccessible])
                      forKey:@"Protection"];
        
        [innerJSON setObject:[NSString stringWithFormat:@"%@",[eachItemFromResults objectForKey:(__bridge id)kSecAttrModificationDate]]
                      forKey:@"Modified Time"];
        
        [innerJSON setObject:[NSString stringWithFormat:@"%@",[eachItemFromResults objectForKey:(__bridge id)kSecAttrCreationDate]]
                      forKey:@"Creation Time"];
        
        [innerJSON setObject:checkForNoDataValue([eachItemFromResults objectForKey:(__bridge id)kSecValueData])
                      forKey:@"Data"];
        
        if (flag_isiOS8) {
            [innerJSON setObject:checkUserPresence((__bridge SecAccessControlRef)([eachItemFromResults objectForKey:(__bridge id)(kSecAttrAccessControl)]))
                          forKey:@"UserPresence"];
            //kSecAttrAccessControl: is for GRANTing. This is a new iOS8 attribute. It allows you to define a fine-grained access control.
            //You use it with the method SecAccessControlCreateWithFlags. For now the enum contains only one value (but there will be room for more configuration) "UserPresence".
        }
        [parentJSON setObject:innerJSON forKey:[NSString stringWithFormat:@"%lu", (unsigned long)++index]];
    
    }
//    NSLog(@"parentJSON = %@",parentJSON);
    /*
    ...
    if (json != nil && error == nil) {
        ...
        fprintf(stderr, "\n\e[0;31mInternal error when preparing JSON output. Error: %s\e[0m\n", [[error localizedDescription] UTF8String]);
    }
}
...
int main(int argc, char *argv[]) {
    @autoreleasepool {
        ...
        if (![[NSString stringWithFormat:@"%s", argv[1]] isEqualToString:@"--action"]) {
            fprintf(stderr, "\e[0;31mInvalid command passed as first argument. Please use --help to get usage information.\e[0m");
            return EXIT_FAILURE;
        }
